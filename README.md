...
- Uses exact package pin `neuralclaw==1.1.6`.
...
- For ChatGPT and Claude session auth, `1.1.6` supports local bootstrap with `neuralclaw session auth chatgpt --stealth` and `neuralclaw session auth claude --stealth` before pasting the resulting session credential into your SaaS deploy form.
